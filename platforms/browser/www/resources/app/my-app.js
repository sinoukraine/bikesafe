...
function getPlusInfo(){
    ...
    if(window.device) {                        
        ...
        localStorage.PUSH_APPID_ID = 'android.app.quiktrak.eu.bikesafe';
        localStorage.DEVICE_TYPE = "android.app.quiktrak.eu.bikesafe";        
    }
}
...
API_URL.URL_USERGUIDE = 'https://sinopacificukraine.com/www/login/bikesafe/manual/bikesafe-app.pdf';
API_URL.URL_SUPPORT = "https://sinopacificukraine.com/support/bikesafe/?name={0}&loginName={1}&email={2}&phone={3}&s={4}";
    ...
    for (var i = assets.length - 1; i >= 0; i--) {        
        if (assets[i].IMEI && assets[i].Props) {
            ...
            for (const key of keys) {
                POSINFOASSETLIST[assets[i].IMEI][key] = list[assets[i].IMEI][key] = assets[i].Props[key];
            }
        }
    }
    localStorage.setItem("COM.QUIKTRAK.LIVE.ASSETLIST", JSON.stringify(list));
